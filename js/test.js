...
function fetchUserData(customerID) {
  return fetch(`http://localhost:5192/api/Users/getById/${customerID}`)
...
function fetchUserAddresses(customerID) {
  return fetch(`http://localhost:5192/api/User/Address/${customerID}`)
    .then((response) => response.json())
    .then((addressData) => {
...
function confirmDelete() {
  ...
  fetch(
    ...
    .then((data) => {
      ...
      // Lấy danh sách địa chỉ sau khi xóa
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  update_button.addEventListener("click", function () {
    ...
    fetch(apiUrl)
      .then((response) => response.json())
      .then((data) => {
        // Kiểm tra xem dataUpdate có giống với data hay không
        const isDataEqual = JSON.stringify(data) === JSON.stringify(dataUpdate);

        if (isDataEqual) {
          showNotification("Vui lòng thay đổi dữ liệu trước khi cập nhật");
          // Thực hiện logic khi dataUpdate giống với data
        } else {
